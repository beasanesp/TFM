...
df_nested_list[['NOR','DEL','DUP']] = pd.DataFrame(df_nested_list.scores.tolist(), index= df_nested_list.index)
# Load data
data = load_csv(path_inputdata, [])
data = data.fillna(0) # Simple imputation

data_out = pd.concat([data.reset_index(drop=True), df_nested_list[['NOR','DEL','DUP']]], axis=1)
#data_out = df_c.drop('batch.1', axis=1)
data_out.to_csv(path_output+filename+'.pred', index=False)