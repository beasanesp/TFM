...
###################################################################
#Asegurarse que la semilla cambia par true undersampling
cat("Seed: ", as.numeric(Sys.time()), "\n")
...
randomUnderSampling <- function(dataframe, target){
  ...
  maj_rows <- dataframe[dataframe[[target]] == majorityclass,]
  sampled_maj <- maj_rows[sample(nrow(maj_rows),tosample),]
  
  return(
    rbind(sampled_maj,dataframe[dataframe[[target]] != majorityclass,])
