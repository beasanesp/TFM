...
# For each df, temporary combine chrom, start, end, gene for merging data
VSF <- VSF %>% unite(chrom, start, end, col = "chrom_start_end", sep = '_')
GC <- GC %>% unite(chrom, start, end, col = "chrom_start_end", sep = '_')
DEPTH <- DEPTH %>% unite(chrom, start, end,  col = "chrom_start_end", sep = '_')

# Combine data by matching unique 'GENE' column
# In three steps
metatable <- left_join(GC, VSF,  by = "chrom_start_end")
metatable <- left_join(metatable, DEPTH, by = "chrom_start_end")

# Remove reduntant columns: DEPTH & WEIGHT from CNVKIT
# Seperate REGION column into seperate chrom, and positions columns
# (replace in a future version)
metatable <- metatable %>% 
  separate('chrom_start_end', into=c("chrom","start","end")) %>%
  mutate(length = as.integer(end)-as.integer(start))


# Convert the START and END values into integers instead of characters
metatable$start <- as.integer(metatable$start)
metatable$end <- as.integer(metatable$end)
